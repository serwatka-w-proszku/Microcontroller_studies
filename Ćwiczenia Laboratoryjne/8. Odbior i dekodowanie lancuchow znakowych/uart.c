...
void Reciever_PutCharacterToBuffer(char cCharacter){
	if(cCharacter==TERMINATOR){
		sReciverBuffer.cData[sReceiverBuffer.ucCharCtr]=NULL;
		sReciverBuffer.eStatus=READY;
		sReceiverBuffer.ucCharCtr = 0;
	}
	else if(sRecieverBuffer.ucCharCtr>RECIEVER_SIZE){
		sReciverBuffer.eStatus=OVERFLOW;
	}
	else{
		sReciverBuffer.cData[sReceiverBuffer.ucCharCtr]=cCharacter;
